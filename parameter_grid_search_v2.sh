#!/bin/bash
#Simple array
jetheight=(5. 10. 17.5 22.5 27.5 35 )
#jetheight=(15 20 25)
...
jet_setting=(0 1 2)
min_scale_SIFT=(0.01 0.1 0.25 0.5 0.7 1. 1.3 1.7)
nr_octaves_SIFT=(3 4 5 6 7)
nr_scales_per_octave_SIFT=(4 5 6 7 8 9)
min_contrast_SIFT=(0.01 0.1 0.25 0.5 0.7 1. 1.3 1.5)
...
for noise_off_counter in "${noise_offset[@]}"
do
  for noise_counter in "${noise_level[@]}"
  do
    for jet_set_counter in "${jet_setting[@]}"
    do
          ...
          do
            ...
            do
              ...
              do
                   ...
                   ./testkeypoints --noiseoff=$noise_off_counter --noisestdv=$noise_counter --method=SWIFT --grid=1 -j $jet_set_counter --input_filename_1=../../data/cloud_overlap_1.pcd --input_filename_2=../../data/cloud_overlap_2.pcd --output_filename=../measurements/SIFTtuning.csv  --min_scale_SIFT=$min_scale_SIFT_counter --nr_octaves_SIFT=$nr_octaves_SIFT_counter --nr_scales_per_octave_SIFT=$nr_scales_per_octave_SIFT_counter --min_contrast_SIFT=$min_contrast_SIFT_counter
                   ./testkeypoints --noiseoff=$noise_off_counter --noisestdv=$noise_counter --method=SWIFT --grid=1 -j $jet_set_counter --input_filename_1=../../data/cloud_nocoor.pcd --input_filename_2=../../data/cloud_overlap__big_2.pcd --output_filename=../measurements/SIFTtuning.csv  --min_scale_SIFT=$min_scale_SIFT_counter --nr_octaves_SIFT=$nr_octaves_SIFT_counter --nr_scales_per_octave_SIFT=$nr_scales_per_octave_SIFT_counter --min_contrast_SIFT=$min_contrast_SIFT_counter
              done
            done
          done
        done
